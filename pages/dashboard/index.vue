    ...
    <div class="tw-relative tw-transition-all">
      ...
      <div class="tw-hidden md:tw-block">
        <div class="tw-py-4 tw-bg-brand-primary-light">
          <div
            class="tw-w-11/12 tw-mx-auto tw-flex tw-justify-between tw-items-center"
          >
            <div class="logo tw-font-bold tw-text-xl sm:tw-text-base">
              <h1 class="tw-text-white">
                Jot<span class="tw-text-brand-primary">Loom</span><span></span>
              </h1>
            </div>
            <!-- User Avatar -->
            <user-avatar
            ...
            <v-btn
              color="primary"
              @click.stop="sideNavOpened = !sideNavOpened"
              size="30"
              icon
            >
      ...
      >
        ...
        <div class="tw-mt-9">
          <h1 class="tw-text-2xl md:tw-text-lg">
            Hello,
            <span class="tw-text-brand-primary"
              >{{ fullName.replace(/ .*/, "") }}!</span
            >
          </h1>
        ...
        >
          ...
          >
            <h1
              class="fira tw-font-semibold tw-text-center tw-text-xl sm:tw-text-sm"
            >
              Add new note
            </h1>
            <div class="tw-text-center tw-mt-4">
              ...
              <v-textarea
                persistent-hint
                hint="click on the mic icon to start/stop speaking"
                :append-inner-icon="
                  isRecording ? 'mdi-stop-circle' : 'mdi-microphone'
                "
                @click:append-inner="toggleRecord"
                rows="8"
                outlined
                label="Note"
...
const fullName = computed(() => user.value?.full_name || "User");
...
const getAllNotes = async () => {
  ...
  try {
    ...
    console.log(error);
    fetchActive.value = false;
    alertText.value = "Failed to fetch notes";
...
const saveNewNote = async () => {
  if (newNote.value.title === "") {
    ...
    try {
      ...
      loader.value.save = false;
      console.log(error);
      alertText.value = "Failed to save note";
      alertColor.value = "red";
      snackbar.value = true;
    }
  }
...
const updateNote = async () => {
  ...
  try {
    ...
    await updateNoteApi(updatedNote.id, updatedNote.title, updatedNote.content);
    ...
    console.log(error);
    loader.value.edit = false;
    alertText.value = "Failed to update note";
    alertColor.value = "red";
    snackbar.value = true;
  }
};
...
const deleteNote = async () => {
  try {
    ...
    await deleteNoteApi(updatedNote.id);
    ...
    console.log(error);
    loader.value.delete = false;
    alertText.value = "Failed to delete note";
    alertColor.value = "red";
    snackbar.value = true;
  }
};
...
const filteredNotes = computed(() => {
  ...
  return mainNotes.value.filter((item) => {
    ...
    const d = new Date(item.createdAt).toDateString().toString();
